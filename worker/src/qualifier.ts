...
export interface LeadFields {
  ...
  /** What they're after: a deployment, a specific build, or "just researching". */
...
export const SUBMIT_LEAD_TOOL = {
  ...
  description:
    ...
    'industry, the workflow that hurts and who owns it, concurrent users, ' +
  ...
  input_schema: {
    ...
    properties: {
      ...
      system_interest: {
        type: 'string',
        description:
          'e.g. "Summit Base", "Summit Ridge", "Summit Pinnacle", "Custom / scoping", or "Just researching".'
      },
...
function tierInfo(s?: string): TierInfo {
  ...
  if (/citadel|custom|scoping|enterprise/.test(v))
    return { points: 3, researching: false, scoping: true }
  ...
  if (/landfall|scout|ranger|pack leader/.test(v))
