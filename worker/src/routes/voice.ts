...
interface VapiMessage {
  ...
  // end-of-call-report
  artifact?: {
    messages?: { role: string; content?: string; message?: string }[]
    recordingUrl?: string
  }
  recordingUrl?: string
  transcript?: string
...
export async function handleVoiceWebhook(request: Request, env: Env): Promise<Response> {
  ...
  if (!env.WEBHOOK_SECRET) {
    return jsonResponse(
      { success: false, error: 'Webhook authentication unavailable.' },
      503,
      origin,
      env
    )
  ...
  if (msg.type === 'end-of-call-report') {
    ...
    if (!leadId) {
      ...
      if (fields && hasLeadData(fields)) {
        ...
        console.log(
          `VOICE_EXTRACT lead=${leadId} score=${scored.score} email=${fields.email ?? 'n/a'}`
        )
      }
    }
...
async function extractLeadFromTranscript(
  ...
  const system =
    ...
    'organization, industry, the workflow they described and whose desk it ' +
    'runs through, whether they can host an on-site immersion, compliance ' +
    'needs, timeline, budget, decision-maker role, etc. Omit anything not ' +
...
async function runBookAppointment(
  ...
  if (!startISO || !name || !email) {
    return {
      ...
      message: 'I need your name, email, and a chosen time to book.'
    }
  }
